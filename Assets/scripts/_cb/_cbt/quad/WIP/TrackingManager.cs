...
public class TrackingManager : MonoBehaviour
{
    ...
    public GameObject p_genericStar;
    ...
    public void InitializeTerrain()
    {
        for (int i = 0; i < bodies.Length; i++)
        {
            if (i == 0)
            {
                GameObject g_newPlanetObj = Instantiate(p_genericStar, Vector3.zero, Quaternion.identity);
                g_newPlanetObj.transform.SetParent(transform.GetChild(i));
                g_newPlanetObj.transform.localPosition = Vector3.zero;

                g_newPlanetObj.GetComponent<Planet>().radius = bodies[i].config.equitorialRadius;

                g_newPlanetObj.GetComponent<Planet>().Initialize(i);

                g_newPlanetObj.GetComponent<Planet>().planetMaterial = starMaterial;
            }
            else
            {
                ...
                if (bodies[i].config.hasSurface) {
                    g_newPlanetObj.GetComponent<Planet>().radius = bodies[i].config.equitorialRadius;

                    g_newPlanetObj.GetComponent<Planet>().Initialize(i);

                    //g_newPlanetObj.GetComponent<test>().Initialize();
                    g_newPlanetObj.GetComponent<test>().bodyIndex = i;
                } else {
                    Destroy(g_newPlanetObj.GetComponent<Planet>());
                    Destroy(g_newPlanetObj.GetComponent<test>());
                }
            }
        }
    }
    ...
    void CreatePlanetIcon(int bodyIndex)
    {
        RectTransform t_newIcon = new GameObject().AddComponent<RectTransform>();
        t_newIcon.gameObject.AddComponent<Image>();
        t_newIcon.GetComponent<Image>().sprite = Sys.sprites[bodies[bodyIndex].config.iconIndex];
        t_newIcon.GetComponent<Image>().color = bodies[bodyIndex].config.mapViewColor;
    ...
    void CreatePlotter(int bodyIndex)
    {
        ...
        t_newPlotter.gameObject.GetComponent<LineRenderer>().material.color = bodies[bodyIndex].config.mapViewColor;
